class StoreSales::StoreSalesController < ApplicationController
  ...
  def show
    ...
    if data_filter === 'today'
      store_sales = StoreSale.where(store_id:).added_today
    elsif data_filter === 'this_week'
      store_sales = StoreSale.where(store_id:).added_this_week
    elsif data_filter === 'past_month'
      store_sales = StoreSale.where(store_id:).added_last_month
    elsif data_filter === 'this_month'
      store_sales = StoreSale.where(store_id:).added_this_month
    elsif data_filter === 'range'
      start_date = params[:start_date]
      end_date = params[:end_date]
      StoreSale.where(store_id:).added_within_range(start_date, end_date)
    else
      store_sales = StoreSale.where(store_id:).order(created_at: :desc)
    end
